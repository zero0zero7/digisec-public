...
CUR_DIR=$(pwd)
FC_DIR=${1:-$HOME/firecracker} #absolute path of firecracker binary

if [ ! -e $FC_DIR ]
then
    sh src/firecracker-binary.sh "$FC_DIR"
fi
...
sudo cp build/kernel/linux-5.10/vmlinux-5.10-x86_64.bin "$CUR_DIR/build/vmlinux-5.10-x86_64.bin"